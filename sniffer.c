...
void usage(FILE *fp, char *progname, int exit_code) {
  ...
  fprintf(fp, " -l, --low-latency  try to enable serial port low-latency mode "
              "(Linux-only)\n");
...
void dump_buffer(uint8_t *buffer, uint16_t length) {
  ...
  if (length > 0) {
    ...
    if (length > 1) {
      fprintf(stdout, "Function code: %02X\n", (uint8_t)buffer[1]);
      if (length > 4) {
        fprintf(stdout, "Data:");
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    if ((res = select(port + 1, &set, NULL, NULL, &timeout)) < 0 &&
        errno != EINTR)
      DIE("select");

    /* there is something to read...  */
    if (res > 0) {
      if ((n_bytes = read(port, buffer + size, MODBUS_MAX_PACKET_SIZE - size)) <
          0)
        DIE("read port");
